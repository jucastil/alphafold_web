...
ls -lh /var/www/html/alphafold/monitor/results/ | grep tar | awk '{print "<tr><td>"$9"</td><td>"$5"</td><td>"$6 " "$7" "$8"</td><td><a href=../monitor/results/"$9">Direct Download</a></td></tr>" }'  >> $RESPATH/table.part
...
echo "<button type=\"button\" style=\"background-color:Orange; color: black;  padding: 12px 20px;  border: none; border-radius: 4px;  cursor: pointer; \"onClick=\"window.location ='../monitor/status.html' \">Back to Status</button>" >> $RESPATH/raw.html
