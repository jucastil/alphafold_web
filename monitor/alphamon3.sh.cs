...
###################################################
#	alphamon2 : generate html output from docker
...
do
   echo "<tr><td>${DPSA_N[$i]}" "</td><td>" "${DPSA_I[$i]}" "</td><td>""${DPSA_C[$i]}" "</td><td>" \
    ...
    `docker ps --no-trunc --format "{{ .Command }}"  --filter "name=${DPSA_N[$i]}"  | awk '{ print $2}' | sed 's|--fasta_paths=/home/alphafold/fasta_path_0/||g'` "</td><td>" \
